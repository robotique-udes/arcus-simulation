#include "QErrocCode.hpp"

QErrocCode::QErrocCode(std::shared_ptr<rclcpp::Node> node_, QWidget* parent_):
    QWidget(parent_),
    _node(node_),
    _odomTimeout(rclcpp::Duration::from_seconds(WATCH_DOG_TIMEOUT_S))
{
    ...
    connect(this, &QErrocCode::updateXYZPositionIU, this, &QErrocCode::onUpdateXYZPositionUI);
}

void QErrocCode::setupUI(void)
{   
...
void QErrocCode::initOdomSubscriber(void)
{
    if (_node)
    {
...
void QErrocCode::CB_odom(nav_msgs::msg::Odometry& msg_)
{
...
void QErrocCode::onUpdateXYZPositionUI(float x_, float y_, float z_)
{
    _ui.title->setText("Car Position: ");
...
void QErrocCode::CB_odomPubCount()
{
    _lastOdomMsgTime = _node->now();
...
void QErrocCode::CB_odomTimeout()
{
