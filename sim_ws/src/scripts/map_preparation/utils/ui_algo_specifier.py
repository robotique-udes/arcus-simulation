...
def pick_algo_zones(occupancy_grid):
    """
    Open an interactive map window to draw one or more polygons and assign an algorithm.

    Controls
    --------
    - Left-click on map: add a vertex to the current polygon.
    - Right-click on map: undo last vertex in the current polygon.
    - Choose either 'disparity' or 'pure_pursuit' from the selector.
    ...
    plt.subplots_adjust(bottom=0.17, left=0.22)
    ...
    ax.set_title(
        "Algorithm Zone Drawer: LEFT-CLICK add vertex  |  RIGHT-CLICK undo point\n"
        "Choose 'disparity' or 'pure_pursuit', then 'Save & Exit'",
        color="white",
        fontsize=11,
        pad=12,
    )
    ...
    ax_new = plt.axes([0.28, 0.02, 0.16, 0.055])
    ...
    ax_algo = plt.axes([0.03, 0.45, 0.15, 0.14], facecolor="#10172a")
    algo_selector = RadioButtons(ax_algo, ("disparity", "pure_pursuit"), active=0)
    for label in algo_selector.labels:
    ...
    def _current_algorithm():
        return algo_selector.value_selected
    ...
    def _draw_saved_polygon(poly_idx, polygon):
        ...
        algo_name = state["polygons"][poly_idx]["algorithm"]
        lbl = ax.text(
            center_col,
            center_row,
            f"P{poly_idx + 1}\n{algo_name}",
    ...
    def _commit_current_polygon():
        ...
        polygon = {"points": list(state["current"]), "algorithm": _current_algorithm()}
        state["polygons"].append(polygon)

        _draw_saved_polygon(len(state["polygons"]) - 1, polygon["points"])
